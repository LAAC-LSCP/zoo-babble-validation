...
knitr::include_graphics("fig_levels.pdf")
...
rownames(myr)<-c("z_lp","z_cp","l_lp","l_cp")
colnames(myr)<-c("AS","LR")
mylb=myhb=myn=myp =myr #this is an ugly way of creating 6 matrices

#to improve legibility of figures, providing the coordinates to offset the text
mytext<-rbind(
  cbind(-.1,-.1,0,-.2), #"z_lp"
  cbind(+.1,+.1,-.1,+.3), #"z_cp"
  cbind(-.1,-.1,0,-.2), #"l_lp"
...
rownames(mytext)<-c("z_lp","z_cp","l_lp","l_cp")
colnames(mytext)<-c("AS.x","AS.y","LR.x","LR.y")
...
for(thisvar in c("z_lp","z_cp","l_lp","l_cp")) {
  ...
  plot(proportions[,thisvar]~proportions$Age, pch=mypch[proportions$Diagnosis],xlab="Age (months)",ylab=prettynames[thisvar],
       col=mycols[proportions$Diagnosis],
         ylim=myrange)
  abline(lm(proportions[,thisvar]~proportions$Age,subset=c(proportions$Diagnosis=="AngelmanSyndrome")),col="black",lty=2)
  ...
  thiscor=cor.test(proportions[proportions$Diagnosis=="AngelmanSyndrome",thisvar],proportions$Age[proportions$Diagnosis=="AngelmanSyndrome"])
    ...
    #add r to figure
  text(
    mean(proportions$Age[proportions$Diagnosis=="AngelmanSyndrome"])+mytext[thisvar,"AS.x"],mean(proportions[proportions$Diagnosis=="AngelmanSyndrome",thisvar])+mytext[thisvar,"AS.y"],
    paste0("r=",myr[thisvar,"AS"]),col="black")
  ...
  thiscor=cor.test(proportions[proportions$Diagnosis!="AngelmanSyndrome",thisvar],proportions$Age[proportions$Diagnosis!="AngelmanSyndrome"])
    ...
    text(mean(proportions$Age[proportions$Diagnosis!="AngelmanSyndrome"])+mytext[thisvar,"LR.x"],
         mean(proportions[proportions$Diagnosis!="AngelmanSyndrome",thisvar])+mytext[thisvar,"LR.y"],
         paste0("r=",myr[thisvar,"LR"]),col="red")
}
#dev.off()
```
...
#repeat for all cor
  ...
  abline(lm(proportions[,"z_cp"]~proportions[,"l_cp"],subset=c(proportions$Diagnosis=="AngelmanSyndrome")),col="black",lty=2)
...
names(recalls)<-stall_r$Lab[stall_r$Lab==stall_r$Zooniverse]
names(recalls)[names(recalls)=="Non-Canonical"]<-"Non-can."
  #c("Canonical","Crying","Junk","Laughing","Non-can.")
recalls=recalls[as.character(tab1[2:6,"X"])]
freqs=table(data_all$Zoon)
names(freqs)[names(freqs)=="Non-Canonical"]<-"Non-can."
