...
An impressive total of `r x["nusers","x"]` individual Zooniverse users provided `r x["njudgments","x"]` judgments on `r x["nchunks","x"]` 500-ms chunks, corresponding to `r dim(data_all)[1]` 11,980 LENA^TM^ segments. For a variety of technical mishaps, XX chunks had to be excluded, leading to `r length(levels(factor(chunks$subject_id)))` chunks and `r length(levels(factor(data_all$segmentId_DB)))` LENA^TM^ segments for which we had five Zooniverse labels. Among these, `r round(sum(chunks$nvotes<2)/dim(chunks)[1]*100,1)`% had no matching labels, `r round(sum(chunks$nvotes==2)/dim(chunks)[1]*100,1)`% had two matching labels (the threshold used for lab-annotated segments), and `r round(sum(chunks$nvotes>=3)/dim(chunks)[1]*100,1)`% had three or more matching labels, or majority agreement. Future work may explore different ways of setting the minimal requirement for convergence, but for further analyses here, we focused on the `r round(sum(chunks$nvotes>=3)/dim(chunks)[1]*100,1)`%  of chunks that *did* have at least three labels in agreement (i.e., majority agreement). This represented `r sum(chunks$nvotes>=3)*5` 135,725 labels for `r sum(chunks$nvotes>=3)` 27,145 chunks, corresponding to `r dim(data_all)[1]` LENA^TM^ segments. Given that each chunk is 500 ms in length, this means about `r (sum(chunks$nvotes>=3)*.5)/60/60` hours of audio data were annotated by 8 different annotators (3 in the laboratory, 5 on Zooniverse).
...
The mean correlations across 50 runs were lower than those recovered using all segments from each child (see Table 2), particularly for the low-risk younger infants, and for Linguistic Proportion. This may indicate that, especially in some groups of infants, 100 segments may not be sufficient to capture a stable estimate of the child's Linguistic and Canonical Proportions. We also observed quite a bit of variance across the 50 runs. For Linguistic Proportion, the range of correlations found for all infants was [`r printp(min(resampling$lp_r_all))`,`r printp(max(resampling$lp_r_all))`]; for the Angelman syndrome group [`r printp(min(resampling$lp_r_as))`,`r printp(max(resampling$lp_r_as))`]; for the low-risk group [`r printp(min(resampling$lp_r_lr))`,`r printp(max(resampling$lp_r_lr))`].  For Canonical Proportion, the range of correlations found for all infants was [`r printp(min(resampling$cp_r_all))`,`r printp(max(resampling$cp_r_all))`]; for the Angelman syndrome group [`r printp(min(resampling$cp_r_as))`,`r printp(max(resampling$cp_r_as))`]; for the low-risk group [`r printp(min(resampling$cp_r_lr))`,`r printp(max(resampling$cp_r_lr))`].
